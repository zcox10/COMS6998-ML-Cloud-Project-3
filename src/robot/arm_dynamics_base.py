...
class ArmDynamicsBase:
    ...
    def __init__(
        ...
        for i in range(0, self.num_links):
            self.link_inertias[i] = (
                self.link_masses[i] * self.link_lengths[i] * self.link_lengths[i] / 12.0
            )
    ...
    def compute_pos(self, pos_0, theta):
        ...
        for i in range(1, self.num_links):
            pos[2 * i : 2 * (i + 1)] = pos[2 * (i - 1) : 2 * i] + np.dot(
                rot(theta[i - 1]), self.link_lengths[i - 1] * xaxis()
            )
        return pos
    ...
    def compute_fk(self, state):
        ...
        pos_ee = pos_ee + np.dot(
            rot(theta[self.num_links - 1]), self.link_lengths[self.num_links - 1] * xaxis()
        )
        return pos_ee
    ...
    def compute_pos_com(self, pos, theta):
        ...
        for i in range(0, self.num_links):
            pos_com[2 * i : 2 * (i + 1)] = pos[2 * i : 2 * (i + 1)] + np.dot(
                rot(theta[i]), 0.5 * self.link_lengths[0] * xaxis()
            )
        return pos_com
    ...
    def compute_energy(self, state):
        ...
        for i in range(0, self.num_links):
            T += 0.5 * self.link_masses[i] * (
                np.linalg.norm(vel_com[2 * i : 2 * (i + 1)]) ** 2
