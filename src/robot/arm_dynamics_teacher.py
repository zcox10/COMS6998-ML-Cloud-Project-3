...
class ArmDynamicsTeacher(ArmDynamicsBase):
    ...
    def constraint_matrices(self, state, action):
        ...
        for i in range(0, self.num_links):
            ...
            # gravity
            if self.gravity:
                cr = cr + (-1 * 9.8 * self.link_masses[i]) * (
                    np.dot(rot(-1 * theta[i]), (-1 * yaxis()))
                )
            # centrifugal force
            cr[0] = cr[0] + (-1) * (
                omega[i] * omega[i] * 0.5 * self.link_lengths[i] * self.link_masses[i]
            )
            if i == 0:
                left_hand = cl
                right_hand = cr
        ...
        for i in range(0, self.num_links):
            ...
            if i < self.num_links - 1:
                # the y component
                cl[0, self.idx_f(i + 1) : self.idx_f(i + 2)] = (
                    self.link_lengths[i] * 0.5 * rot(q[i + 1])[1, :]
                )
            left_hand = np.concatenate((left_hand, cl))
            cr = np.zeros((1, 1))
            right_hand = np.concatenate((right_hand, cr))
        ...
        tau = action
        for i in range(self.num_links):
            right_hand[self.idx_tau_eqbm(i), 0] += (
                tau[i + 1] if i < self.num_links - 1 else 0.0
            ) - tau[i]
    ...
    def integrate_euler(self, state, a, qdd, dt):
        ...
        pos_0_new = (
            pos_0 + 0.5 * (np.dot(rot(theta[0]), vel_0) + np.dot(rot(theta_new[0]), vel_0_new)) * dt
        )
